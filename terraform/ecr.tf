...
resource "null_resource" "docker_build_and_push" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      docker tag ${each.value.repository_name}:latest ${data.aws_caller_identity.current.account_id}.dkr.ecr.${var.aws_region}.amazonaws.com/${each.value.repository_name}:latest

      docker push ${data.aws_caller_identity.current.account_id}.dkr.ecr.${var.aws_region}.amazonaws.com/${each.value.repository_name}:latest
    EOT
  }

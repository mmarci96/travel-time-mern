...
export const deletePost = async (post_id: Types.ObjectId, author_id: Types.ObjectId) => {
    if(!post_id) throw new Error("No post id provided!")
    
    const post = await PostModel.findOne({ _id: post_id, author_id })
    
    if(!post) throw new Error("Post not found or you don't have permission to delete it")

...
import { NotificationModel, NotificationType, TargetType } from '../model/NotificationModel';
...
export const markRead = async (notificationId: Types.ObjectId) => {
    ...
    const notification = await NotificationModel.findOneAndUpdate({_id: notificationId}, { read: true })
    if(!notification){
        throw new BadRequestError({
            code: 400,
